...
User uploads any image → pipeline processes it → shows results
"""
...
def to_thermal(img_gray):
    """Convert grayscale image to thermal-like float array."""
    return img_gray.astype(np.float32) / 255.0


def calibrate(frame):
    """Simulate NUC calibration — normalize and remove fixed pattern."""
    # Simulate gain/offset correction
...
def deterministic_process(frame):
    """Bilateral filter + CLAHE contrast enhancement."""
...
def ai_enhance(frame):
    """2x upscale + sharpening (trained params: sharpen=1.3, blur=3)."""
    H, W = frame.shape
    u8 = (frame * 255).clip(0, 255).astype(np.uint8)
...
def detect_objects(frame):
    """Detect warm objects using multi-threshold contour detection."""
    u8 = (frame * 255).clip(0, 255).astype(np.uint8)
    H, W = u8.shape
    detections = []
    all_bboxes = []
    ...
    for thresh_pct in [1.05, 1.15, 1.25, 1.35]:
        ...
        for cnt in contours:
            ...
            # Classify
            if aspect < 0.75 and area > 500:
                label = "Person"
                color = (0, 255, 128)
            elif aspect > 1.3 and area > 1000:
                label = "Vehicle"
                color = (0, 128, 255)
            elif area < 2000:
                label = "Animal"
    ...
    # NMS
    if all_bboxes:
        indices = cv2.dnn.NMSBoxes(all_bboxes, all_confs, 0.15, 0.3)
        if len(indices) > 0:
...
def apply_colormap(frame_norm):
    """Apply thermal inferno colormap."""
...
def draw_detections(colored_frame, detections):
    """Draw detection boxes on colored frame."""
...
def frame_to_base64(frame_bgr):
    """Convert BGR frame to base64 PNG string."""
    _, buffer = cv2.imencode('.png', frame_bgr)
    return base64.b64encode(buffer).decode('utf-8')


def run_pipeline(image_bytes):
    """Run full pipeline on uploaded image bytes."""
    # Decode image
